...
In order to fix these issues DICE aims to produce diverse and feasible counterfactuals. The objective is to generate a set of counterfactuals ${c_1, c_2, dots , c_k }$ for some machine learning model $f$ on some instance $x$, in order to do this DICE includes a diversity and feasibility metric into their generation method. Some features might however might be mutable in some range, meaning that some feature cannot go under or over a limit set by the user. DICE allows for such constraint as well.  @mothilalExplainingMachineLearning2020
