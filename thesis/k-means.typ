...
Centroid clustering is an objective concerned with finding groups of data based on optimizing a set of cluster centers. In such a setting any point is assigned to it's closest cluster center based on some distance metric. As our objective is to find counterfactuals for such methods we will now look into the most well known method for finding such clusters, that being the k-means objective. K-means was originally created in @lloydLeastSquaresQuantization1982 and then generalized for clustering in @macqueenMethodsClassificationAnalysis1967. The method described in these papers are what's commonly known as Lloyds's method, later this was further improved upon with better initializing of the centers in  @arthurKmeansAdvantagesCareful who introduced the K-means++ algorithm.
